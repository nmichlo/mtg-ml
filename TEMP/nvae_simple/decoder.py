...
from examples.nvae.common import DecoderResidualBlock
from examples.nvae.common import ResidualBlock
from examples.nvae.common import Swish
from examples.nvae.losses import kl_2
from examples import iter_pairs
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, channels):
        ...
        self.module_list = nn.ModuleList([
            UpsampleBlock(inp, out)
            for inp, out in iter_pairs(channels)
        ])
...
class Decoder(nn.Module):
    ...
    def forward(self, z, xs=None):
        ...
        for i in range(len(self.decoder_residual_blocks)):
            ...
            z = Normal(mu, torch.exp(0.5 * log_var)).rsample()
